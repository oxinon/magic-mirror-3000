(() => {
    ...
    // auf API_PORT statt dem Port dieser Seite (5033).
    ...
    async function loadTitle() {
        try {
            ...
            const title = data.ok && data.config?.widgets?.shoppingList?.title;
            if (title) {
    ...
    async function loadItems({ silent } = {}) {
        try {
            const data = await fetchJson(`${API_BASE}/api/shopping-list`);
